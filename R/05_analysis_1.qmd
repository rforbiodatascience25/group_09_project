...
```{r, fig.width= 11, fig.height= 6, dpi = 200}
...
# --- Pre-processing for Ivan's plots ---
# Ensure we have consistent column names and a binary response variable
df_analysis <- experiment_data

if("sampleID" %in% names(df_analysis)) df_analysis <- rename(df_analysis, patient_id = sampleID)
if("Protein" %in% names(df_analysis))  df_analysis <- rename(df_analysis, ORF = Protein)
if("Peptide" %in% names(df_analysis))  df_analysis <- rename(df_analysis, peptide = Peptide)
if("HLA" %in% names(df_analysis))      df_analysis <- mutate(df_analysis, HLA = str_remove(HLA, "HLA-"))

# Create response_binary if missing (p < 0.05)
if(!"response_binary" %in% names(df_analysis)) {
  if("p" %in% names(df_analysis)) {
      df_analysis <- df_analysis %>% mutate(response_binary = if_else(p < 0.05, 1, 0))
  }
}
